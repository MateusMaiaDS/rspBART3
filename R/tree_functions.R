...
updateGamma <- function(tree,
  ...
  for(i in 1:length(t_nodes_names)){
    ...
    # Computing mean and sd from the intercep
      gamma_mean <- s_gamma_inv*(r_sum-crossprod(leaf_beta,matrix(colSums(data$D_train[cu_t$train_index,leaf_basis_subindex,drop = FALSE]),ncol = 1)))
      #maybe I don't need to use the matrix there, and ncol=1 too
    } else {
      gamma_mean <- s_gamma_inv*r_sum
    }
